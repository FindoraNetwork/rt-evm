...
pub struct RTEvmWeb3RpcFilter {
    sender: Sender<Command>,
}

#[async_trait]
impl Web3FilterServer for RTEvmWeb3RpcFilter {
    ...
    Adapter: APIAdapter + 'static,
{
    ...
    fn check_hubs(&mut self) {
        ...
        self.blocks_hub.retain(|_, (_, time)| {
            now.saturating_duration_since(*time) < Duration::from_secs(40)
        });
    ...
    async fn handle(&mut self, cmd: Command) {
        match cmd {
            ...
            Command::Uninstall((id, sender)) => {
                let removed = self.blocks_hub.remove(&id).is_some()
    ...
    async fn impl_filter(
        &mut self,
        id: U256,
        sender: oneshot::Sender<RpcResult<FilterChanges>>,
    ) {
    ...
    async fn filter_block(&mut self, id: &U256) -> Vec<H256> {
        ...
        let mut block_hashes =
            Vec::with_capacity((latest.header.number - *start) as usize);
