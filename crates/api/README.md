# RTEvm JSON-RPC Protocols

This section lists the RTEvm JSON-RPC API endpoints.

RTEvm JSON-RPC allow you to interact with a local or remote rt-evm node using HTTP, IPC or WebSocket.
...
Submits a pre-signed transaction for broadcast to the RTEvm network.
...
Used for submitting a proof-of-work solution. Since the rt-evm does not use it, so returns true forever.
...
##### Returns
  ...
  Boolean - returns true if the provided solution is valid, otherwise false.Since the rt-evm does not use it, so return true forever.
...
Returns the chain_id of rt-evm network.
...
Returns true if client is actively listening for network connections. Always return true in RTEvm network.
...
*Note:* Always return false in RTEvm network.
...
##### Returns
 ...
 Boolean - returns true of the client is mining, otherwise false. Always return false in RTEvm network.
...
*Note:* Always return 1 in RTEvm network.
...
*   `transaction_type`: [`U64`](#type-U64) - 64-bit integer that represents the type of the transaction. RTEvm has evolved to support 3 types of transactions: Legacy is 0x0, Eip2930  is 0x1 and Eip1559  is 0x2.
...
*   `type_`: [`U64`](#type-U64) -  64-bit integer that represents the type of the transaction. RTEvm has evolved to support 3 types of transactions: Legacy is 0x0, Eip2930  is 0x1 and Eip1559  is 0x2.
