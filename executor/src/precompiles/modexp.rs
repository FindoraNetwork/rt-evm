...
impl PrecompileContract for ModExp {
    ...
    fn gas_cost(input: &[u8]) -> u64 {
        ...
        let dynamic_gas = large_number.multiplication_complexity()
            * large_number.iterator_count()
            / 3u64;
...
impl LargeNumber {
    ...
    fn parse(input: &[u8]) -> Result<Self, PrecompileFailure> {
        ...
        let base_size =
            Self::parse_big_uint(&input[0..32], true)?.unwrapped_as::<usize>();
        let exponent_size =
            Self::parse_big_uint(&input[32..64], true)?.unwrapped_as::<usize>();
        let modulo_size =
            Self::parse_big_uint(&input[64..96], true)?.unwrapped_as::<usize>();
    ...
    fn iterator_count(&self) -> u64 {
        let iter_count = if self.e_size <= 32 && self.exponent == Integer::ZERO {
            ...
            (8 * (self.e_size - 32))
                + ((self.exponent.clone().bitand(max_256_bit_uint)).significant_bits()
                    - 1) as usize
        };
    ...
    fn parse_big_uint(
        input: &[u8],
        is_parse_size: bool,
    ) -> Result<Integer, PrecompileFailure> {
